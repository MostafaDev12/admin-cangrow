...
@section('content')
    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="card-body">
              <form id="geniusform" action="{{route('admin-site_stats-update',$data->id)}}" method="POST" enctype="multipart/form-data">
                {{csrf_field()}}
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="section" id="section" class="form-control">
                                        <option value="home_hero" {{ $data->section == 'home_hero' ? 'selected' : '' }}>إحصائيات الهيرو - الصفحة الرئيسية</option>
                                        <option value="home_about" {{ $data->section == 'home_about' ? 'selected' : '' }}>إحصائيات من نحن - الصفحة الرئيسية</option>
                                        <option value="about_page" {{ $data->section == 'about_page' ? 'selected' : '' }}>إحصائيات صفحة من نحن</option>
                                    </select>
                                </div>

                                <div class="mb-3">
                                    <label for="icon" class="form-label">الأيقونة (FontAwesome)</label>
                                    <input type="text" class="form-control" name="icon" id="icon" value="{{ $data->icon }}" placeholder="مثال: fas fa-shield-alt">
                                </div>

                                <div class="mb-3">
                                    <label for="value" class="form-label">القيمة</label>
                                    <input type="text" class="form-control" name="value" id="value" value="{{ $data->value }}" placeholder="مثال: 5000+">
                                </div>
